...
if __name__ == '__main__':
    with open('output/lena.12.bin', 'rb') as file:
        text = file.read()
    ...
    with open('huffman_out/lena.12.bin', 'wb') as file:
        file.write(encoded_text)
    with open('huffman_out/lena.12.cb', 'w') as file:
        file.write(json.dumps(codebook))

    with open('huffman_out/lena.12.bin', 'rb') as file:
        text = file.read()
    with open('huffman_out/lena.12.cb', 'r') as file:
        codebook = json.loads(file.read())

    decoded_text = decode(text, codebook)
    with open('huffman_proof/lena.12.bin', 'wb') as file:
